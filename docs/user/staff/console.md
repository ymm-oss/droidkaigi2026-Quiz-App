...
フォルダ（クイズセット）を選び、問題一覧の確認・編集・**フォルダごとの公開**を行います。複数フォルダを同時に公開でき、参加者アプリでは名称で選べます。トップバーではサイト全体の受付公開（`sitePublished`）を切り替え、**参加者アプリ**から配布 URL をコピー／ブラウザで開けます。問題一覧ヘッダの「プレビュー」で参加者画面を試遊できます。
...
参加者アプリの選択肢にフォルダを足す／外すとき、確認ダイアログが表示されます。複数フォルダを同時に公開できます。
