...
Select a folder (quiz set), edit questions, and **publish folders independently**. Several folders can be published at once; participants pick them by name. The top bar toggles site-wide intake (`sitePublished`) and **参加者アプリ** copies/opens distribution URLs. **Preview** on the question list plays the participant UI.
